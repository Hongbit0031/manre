<template>
    <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter="getDataList()">
            <el-form-item>
                <el-button
                    v-if="isAuth('admin:link:save')"
                    type="primary"
                    @click="addOrUpdateHandle()"
                    >新增</el-button
                >
                <el-button
                    v-if="isAuth('admin:link:delete')"
                    type="danger"
                    @click="deleteHandle()"
        ...
        >
            ...
            >
                <template v-slot="scope">
                    <div>
                        <el-tag v-if="scope.row.cateId == 0" type="success"
                            >广场页</el-tag
                        >
                        <el-tag v-else type="primary">个人页</el-tag>
                    </div>
                </template>
            </el-table-column>
            <el-table-column
...
function getDataList() {
    ...
    proxy
        .$http({
            url: proxy.$http.adornUrl("/admin/link/list"),
            method: "get",
...
function deleteHandle(id) {
    ...
    ElMessageBox.confirm(
        ...
        .then(() => {
            proxy
                .$http({
                    url: proxy.$http.adornUrl("/admin/link/delete"),
                    method: "post",
