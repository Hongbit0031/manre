<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter="getDataList()">
      <el-form-item>
        <el-button v-if="isAuth('admin:appversion:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('admin:appversion:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      >
        <template v-slot="scope">
          <div>
            <el-tag v-if="scope.row.isForce == 1" type="danger">强制更新</el-tag>
            <el-tag v-else type="info">非强制</el-tag>
          </div>
        </template>
      </el-table-column>
      <el-table-column
...
<script setup>
  ...
  function getDataList(){
      ...
      proxy.$http({
          url: proxy.$http.adornUrl('/admin/appversion/list'),
          method: "get",
  ...
  ElMessageBox.confirm(`确定对[id=${ids.join(",")}]进行[${id ? "删除" : "批量删除"}]操作?`,"提示",{
                ...
                proxy.$http({
                  url: proxy.$http.adornUrl('/admin/appversion/delete'),
                  method: "post",
