...
export default{
	// 1.获取当前时间 格式 yyyy/mm/dd HH:MM:SS (兼容iOS)
	...
	TimeDifferenceSeconds(start, end){
		...
		//将日期字符串中的"-"替换为"/"以兼容iOS
		var stime = new Date(start.replace(/-/g, '/')).getTime();
	...
	TimeDifferenceDays(start, end) {
	   //定义两个变量start,end分别保存开始和结束时间
	   //将日期字符串中的"-"替换为"/"以兼容iOS
	    var stime = new Date(start.substring(0,10).replace(/-/g, '/')).getTime();
	    var etime = new Date(end.substring(0,10).replace(/-/g, '/')).getTime();
	    var usedTime = etime - stime;  //两个时间戳相差的毫秒数
